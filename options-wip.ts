import chrome from '@extend-chrome/storage';

var REG_STRIP = /^[\r\t\f\v ]+|[\r\t\f\v ]+$/gm;
...
// recordKeyPress {{{
function recordKeyPress(e: KeyboardEvent) {
  if (
    (e.key>= '0' && e.key <= '9') ||
    (e.key >= 'a' && e.key <= 'z') ||
    (e.key >= 'A' && e.key <= 'Z') ||
    ...
    const shift = target.nextElementSibling as HTMLInputElement;
    const ctrl = shift.nextElementSibling?.nextElementSibling as HTMLInputElement;
...
function updateShortcutInputText(inputId: string, key: string) {
  const el = document.getElementById(inputId) as HTMLInputElement;
  el.value = key;
  // document.getElementById(inputId).keyCode = keyCode; // FMFOO
}

function updateCustomShortcutInputText(inputItem: HTMLInputElement, key: string) {
  inputItem.value = key;
  // inputItem.keyCode = keyCode; // FMFOO
}

function show_experimental() {
  (document.querySelectorAll('.customForce') as NodeListOf<HTMLElement>)
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function eventCaller(
    ...
    className: string,
    func: (event: KeyboardEvent | FocusEvent) => void,
  ) {
...
function inputFilterNumbersOnly(e: KeyboardEvent) {
}
...
function inputBlur(e) {
  ...
  document.addEventListener('focus', (event) => {
    if (!(event.target as HTMLElement).classList.contains('customKey')) return;
  ...
  document.addEventListener('blur', (event) => {
    ...
    (event.target as HTMLInputElement).value = TARGETS.get(event.target as HTMLElement)?.key!;
  });
  ...
  document.addEventListener('click', (event) => {
    if (!(event.target as HTMLElement).classList.contains('removeParent')) return;

    (event.target as HTMLElement).parentElement?.remove();
  });

  document.addEventListener('change', (event) => {
    if (!(event.target as HTMLElement).classList.contains('customDo')) return;

    const parent = (event.target as HTMLElement).parentNode;
    const customValue = parent?.querySelector('.customValue') as HTMLInputElement;

    if (customActionsNoValues.includes((event.target as HTMLInputElement).value)) {
