...
runner.test('startHidden setting should correctly control initial controller visibility', async () => {
  const config = window.VSC.videoSpeedConfig;
  await config.load();
  ...
  const visibleController = new window.VSC.VideoController(mockVideo, null, config, actionHandler);
  ...
  const hiddenController = new window.VSC.VideoController(mockVideo2, null, config, actionHandler);
